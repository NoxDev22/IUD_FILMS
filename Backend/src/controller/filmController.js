import { FILM_MODEL } from "../model/filmModel.js";

export class FILM_CONTROLLER {
  static async getAll(req, res) {
    try {
      const movies = await FILM_MODEL.getAll(req.query);
      if (!movies) {
        return res
          .status(404)
  ...
  static async getById(req, res) {
    try {
      ...
      const filmById = await FILM_MODEL.getById(filmId);
  ...
  static async create(req, res) {
    try {
      const createdFilm = await FILM_MODEL.create(req.body);
  ...
  static async update(req, res) {
    try {
      const { filmId } = req.params;
      const updatedFilm = await FILM_MODEL.update(filmId, req.body);
  ...
  static async delete(req, res) {
    try {
      const { filmId } = req.params;
      const deletedFilm = await FILM_MODEL.delete(filmId);
